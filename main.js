...
// key to store/access local data
...
function save() {
  function sortArr() {
    ...
    // reassign id tag given new position
    items.forEach((item) => {
  ...
  function render() {
    ...
    items.forEach((item) => {
      ...
      checkboxNode.onclick = function () {
        const strikeParent = checkboxNode.closest('.item');
      ...
      deleteNode.onclick = function () {
        // re-renders page
        // gets closest parent div of the garbage can icon, then gets its data tag
        // (which corresponds to its location in the Items array)
        const goner = Number(this.closest('.item').getAttribute('data-item-tag'));
        // removes that item from the array
        items.splice(goner, 1);
        // loops through the array and updates the tag attribute of each object,
        // using new position in array
        items.forEach((item) => {
          item.tag = items.indexOf(item);
        });
...
inputDetails.addEventListener('keyup', (e) => {
  if (e.keyCode === 13) {
    e.preventDefault();
    document.getElementById('submit-item').click();
  }
